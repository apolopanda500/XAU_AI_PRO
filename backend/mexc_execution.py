...
class MexcExecutionAdapter:
    def __init__(self, market: str = "spot") -> None:
        ...
        self.base=os.getenv("MEXC_SPOT_BASE_URL" if self.spot else "MEXC_FUTURES_BASE_URL", "https://api.mexc.com" if self.spot else "https://contract.mexc.com").rstrip("/")
    ...
    def _signed(self, method: str, path: str, params: dict[str, object]) -> dict:
        ...
        req=Request(self.base+path, data=urlencode(query).encode() if method=="POST" else None, headers={"X-MEXC-APIKEY":self.key,"Content-Type":"application/x-www-form-urlencoded"}, method=method)
        if method=="GET": req.full_url=self.base+path+"?"+urlencode(query)
        with urlopen(req, timeout=10) as response: data=json.loads(response.read().decode())
    ...
    def execute(self, order: dict, *, explicit_authorization: bool) -> dict:
        if not explicit_authorization: raise MexcExecutionError("autorização explícita ausente")
        if os.getenv("XAU_ENABLE_MEXC_EXECUTION","0")!="1": return {"ok":False,"status":"blocked","reason":"execução MEXC desativada por padrão","withdrawals_enabled":False,"order":order}
        path="/api/v3/order" if self.spot else "/api/v1/private/order/submit"
        payload=order if self.spot else {"symbol":order["symbol"],"price":order.get("price",0),"vol":order["quantity"],"side":1 if order["side"]=="BUY" else 3,"type":5 if order["type"]=="MARKET" else 1,"openType":1,"externalOid":order["newClientOrderId"]}
        result=self._signed("POST",path,payload); return {"ok":True,"status":"submitted","ticket":result.get("orderId"),"broker":"mexc","market":self.market,"response":result,"withdrawals_enabled":False}
